...
async def create_pay_order(
    ...
    if not is_configured():
        raise HTTPException(
            status_code=503,
            detail="Payments not configured. Admin must add Razorpay keys."
        )
    ...
    if payload.purpose == "unlock":
        ...
        if prop["owner_id"] == user["id"]:
            raise HTTPException(
                status_code=400,
                detail="Cannot unlock your own listing"
            )

        already = await db.unlocks.find_one({
            "tenant_id": user["id"],
        ...
        if await is_premium_active(user["id"]):
            raise HTTPException(
                status_code=400,
                detail="Premium already unlocks all contacts"
            )

        amount = PLANS["unlock"]["amount"]

        receipt = (
            f"unlk_{payload.property_id[:8]}_"
            f"{user['id'][:6]}"
        )
...
async def _grant_after_success(
    user_id: str,
    purpose: str,
    ...
    # ---------------------------------------------------------
    # UNLOCK PROPERTY CONTACT
    # ---------------------------------------------------------
    if purpose == "unlock" and property_id:

        exists = await db.unlocks.find_one({
            "tenant_id": user_id,
...
# ============================================================
# NORMAL FRONTEND PAYMENT VERIFICATION
# ============================================================
...
async def my_subscription(
    ...
    active = await is_premium_active(
        user["id"]
    )
